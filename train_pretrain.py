"""Contrastive Pre-Training Script
...
group.add_argument(
    ...
    choices=["info_nce"],
    default="info_nce",
    help="Loss function (default: info_nce)",
)
...
def main():
    ...
    if args.loss == "info_nce":
        train_loss_fn = InfoNCE(**args.loss_kwargs).to(device=device)
        validate_loss_fn = InfoNCE(**args.loss_kwargs).to(device=device)
    else:
...
def train_one_epoch(
    ...
    for batch_idx, (wsi_features, rna_features) in enumerate(loader):
        ...
        def _forward():
            with amp_autocast():
                wsi_embeddings, rna_embeddings = model(
                    wsi_features, rna_features  # noqa: B023
                )
                loss = loss_fn(wsi_embeddings, rna_embeddings)
...
def validate(
    ...
    with torch.no_grad():
        for batch_idx, (wsi_features, rna_features) in enumerate(loader):
            ...
            with amp_autocast():
                wsi_embeddings, rna_embeddings = model(wsi_features, rna_features)
                loss = loss_fn(wsi_embeddings, rna_embeddings)
