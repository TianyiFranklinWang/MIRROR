...
from datasets import TCGAWSIRNASurvivalDataset
from losses import CrossEntropySurvLoss, NLLSurvLoss
from utils import class_balanced_sampler
...
parser.add_argument("--survival-csv", metavar="PATH", help="path to survival csv file")
parser.add_argument(
...
def main():
    ...
    ), "Number of classes must be equal to number of bins."
    ...
    dataset_train = TCGAWSIRNASurvivalDataset(
    ...
    dataset_train.train()
    if args.val:
        dataset_eval = TCGAWSIRNASurvivalDataset(
            wsi_feature_dir=args.wsi_feature_dir,
            rna_feature_csv=args.rna_feature_csv,
            survival_csv=args.survival_csv,
            num_wsi_feature_tokens=args.num_wsi_feature_tokens,
    ...
    if args.loss == "ce_surv":
        train_loss_fn = CrossEntropySurvLoss().to(device=device)
        validate_loss_fn = CrossEntropySurvLoss().to(device=device)
    elif args.loss == "nll_surv":
        train_loss_fn = NLLSurvLoss(alpha=args.loss_alpha).to(device=device)  # type: ignore[assignment]
        validate_loss_fn = NLLSurvLoss(alpha=args.loss_alpha).to(device=device)  # type: ignore[assignment]
    else:
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if not args.wsi_feature_only:
            wsi_features, rna_features, labels, event_times, censorships = batch
        else:
            wsi_features, labels, event_times, censorships = batch

        labels, event_times, censorships = (
            labels.squeeze(),
            event_times.squeeze(),
            censorships.squeeze(),
        )
        wsi_features, labels, event_times, censorships = (
            wsi_features.to(device),
            labels.to(device),
            event_times.to(device),
        ...
        def _forward():
            ...
            if amp_autocast is not suppress:
                with amp_autocast(
                    enabled=False
                ):  # Disable amp for loss computation to increase numerical stability
                    loss = loss_fn(output.float(), labels, censorships)  # noqa: B023
            else:
                loss = loss_fn(output.float(), labels, censorships)  # noqa: B023
...
def validate(
            ...
            with amp_autocast():
                ...
                if amp_autocast is not suppress:
                    with amp_autocast(
                        ...
                        loss = loss_fn(output.float(), labels, censorships)
                else:
                    loss = loss_fn(output.float(), labels, censorships)
                ...
                all_censorships.append(censorships.detach().cpu().numpy())
                all_event_times.append(event_times.detach().cpu().numpy())
    ...
    c_index = concordance_index_censored(
        np.concatenate(all_censorships).astype(bool),
        np.concatenate(all_event_times),
        np.concatenate(all_risk_scores),
        tied_tol=1e-8,
    )[0].item()
    if args.distributed:
        c_index = utils.reduce_tensor(
            torch.tensor(c_index).to(device), args.world_size
        ).item()

    if utils.is_primary(args):
        _logger.info(f" * Loss: {losses_m.avg:>7.3f}  C-Index: {c_index:>7.3f}")
